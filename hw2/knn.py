...
class knn:
    ...
    def predict_single(self, input_point):
        ...
        distances = [
            self.calculate_distance(input_point, self.training_set.iloc[[i]])
            for i in range(self.training_set.shape[0])
        ]
        nearest_indices = np.argsort(distances)[: self.k]
        nearest_classes = [
            self.training_set.iloc[i][self.predicting_feature] for i in nearest_indices
        ]
